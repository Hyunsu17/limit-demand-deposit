...
| **Week 3** | 🔄 진행중 (7/19~, **달력상 Week 6 — 3주 지연**) — **입금 경로가 컨트롤러까지 관통**. `DepositLimit` VO(2중 한도), Flyway V4·V5·**V6**, 거래·정책 엔티티 + 포트, 입금 조회 포트 2종, Service 오케스트레이션(TX1/TX2 분리 + 결과 객체 거절) + 컨트롤러 + 전역 예외 핸들러, **입금 통합 테스트 2케이스**(8/10). 전체 스위트 82개 그린(1 skip) |
| **다음 단계** | ✅ 스코프 절단 완료(8/19) → **잔여 3주분 확정**: ① 입금 마무리 — **비관적 락 인프라 테스트**(스레드 2개 필요, 기존 `@DataJpaTest` 4종 세트로 불가) → `withdraw` 잔액검증 → 계좌 상태전이 메서드 / ② **이자 계산 — 계산 로직 + 경계값 단위 테스트만**(배치·스케줄러·스냅샷 제외) / ③ 정리·문서·**설명 리허설**(완료 기준이 "20분 설명"이므로 부가작업이 아니라 최종 산출물) |
| **미해결(주의)** | **비관적 락 쿼리는 JPQL 파싱까지만 검증** — 실제 락 획득 테스트 없음(월누계 SUM은 8/3 검증 완료) / 🔎 **V6 CHECK에 구멍** — `FAILED` + `fail_reason = ''`(빈 문자열)이 통과한다. `IS NOT NULL`만 보므로 길이 0은 안 걸림. 빈 사유는 추적 근거 부재와 같아 취지에 어긋나지만 `@Enumerated(STRING)`이 `''`를 안 만들어 실위험 낮음 → **V7로 조이지 않고 유지**(8/3 결정, 범위 통제). 참고: 오라클은 `''`=NULL, PostgreSQL은 구분 / 08-03 변경분 중 Service·컨트롤러는 통합 테스트 2케이스가 덮음(8/10). **전역 예외 핸들러는 여전히 테스트 없음** / `withdraw` 잔액 검증 없음(음수 가능) / 계좌 상태전이 메서드(해지·동결) 부재 → `AccountTest` 비활성 케이스 `@Disabled` 보류 중 / `deposit_limit_policy` D01 시드 금액은 **임시값** / 🕛 **시각 일관성 미판단** — 한 입금 처리에서 "지금"을 3곳이 각각 구함(`DepositProcessService`·`Account.deposit()`·`TransactionHistory` 생성자), 자정 경계에서 당월 누계 기준월과 `txn_dt`가 갈릴 수 있음 / 🧾 `raw_data`는 DTO 재직렬화라 미지 필드가 원본에서 소실 |
