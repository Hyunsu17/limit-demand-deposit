...
| 항목 | 내용 |
|------|------|
| **완료 기준** | 🎯 **"계좌개설과 입금을 각각 20분 설명할 수 있는 상태"** (2026-08-19 확정). 제품("수신 도메인 구현")이 아니라 포트폴리오 기준 — 행동원칙 4번(Goal-Driven)을 코드가 아닌 프로젝트 자체에 적용 |
...
| **Week 3** | 🔄 진행중 (7/19~, **달력상 Week 6 — 3주 지연**) — **입금 경로가 컨트롤러까지 관통**. `DepositLimit` VO(2중 한도), Flyway V4·V5·**V6**, 거래·정책 엔티티 + 포트, 입금 조회 포트 2종, Service 오케스트레이션(TX1/TX2 분리 + 결과 객체 거절) + 컨트롤러 + 전역 예외 핸들러, **입금 통합 테스트 2케이스**(8/10), **비관적 락 실차단 테스트 A**(8/19). 전체 스위트 83개 그린(1 skip) |
| **다음 단계** | ✅ 스코프 절단 완료(8/19) → **잔여 3주분 확정**: ① 입금 마무리 — 비관적 락 테스트 **A 완료(8/19)**, 남은 **C·B** → `withdraw` 잔액검증 → 계좌 상태전이 메서드 / ② **이자 계산 — 계산 로직 + 경계값 단위 테스트만**(배치·스케줄러·스냅샷 제외) / ③ 정리·문서·**설명 리허설**(완료 기준이 "20분 설명"이므로 부가작업이 아니라 최종 산출물) |
| **미해결(주의)** | 🔒 **비관적 락 실차단 검증 완료(8/19)** — `AccountPessimisticLockTest` A케이스. 변이 실험(`@Lock` 제거 → 빨간불)까지 확인. **실측에서 드러난 것: 이 쿼리는 `FOR UPDATE` 가 아니라 `FOR NO KEY UPDATE` 로 나간다**(Hibernate 6 + PostgreSQL). 직렬화엔 충분하고 `account_ledger` 에 FK 가 없어 실질 차이 없음 — 기존 주석이 부정확했던 것을 테스트가 잡음. 예외 타입도 실측: 정확히 `PessimisticLockingFailureException`(`CannotAcquireLockException` 아님). **남은 케이스: C(다른 계좌는 안 막힌다 — 락의 범위) · B(동시 입금 잔액 합계)** / 🔎 **V6 CHECK에 구멍** — `FAILED` + `fail_reason = ''`(빈 문자열)이 통과한다. `IS NOT NULL`만 보므로 길이 0은 안 걸림. 빈 사유는 추적 근거 부재와 같아 취지에 어긋나지만 `@Enumerated(STRING)`이 `''`를 안 만들어 실위험 낮음 → **V7로 조이지 않고 유지**(8/3 결정, 범위 통제). 참고: 오라클은 `''`=NULL, PostgreSQL은 구분 / 08-03 변경분 중 Service·컨트롤러는 통합 테스트 2케이스가 덮음(8/10). **전역 예외 핸들러는 여전히 테스트 없음** / `withdraw` 잔액 검증 없음(음수 가능) / 계좌 상태전이 메서드(해지·동결) 부재 → `AccountTest` 비활성 케이스 `@Disabled` 보류 중 / `deposit_limit_policy` D01 시드 금액은 **임시값** / 🕛 **시각 일관성 미판단** — 한 입금 처리에서 "지금"을 3곳이 각각 구함(`DepositProcessService`·`Account.deposit()`·`TransactionHistory` 생성자), 자정 경계에서 당월 누계 기준월과 `txn_dt`가 갈릴 수 있음 / 🧾 `raw_data`는 DTO 재직렬화라 미지 필드가 원본에서 소실 |
| **진행 방식 합의** | 인프라·통합 테스트는 하나씩 설명 → 함께 작성 → 리뷰 (일괄 작성 금지, 2026-07-11 합의) |
| **grill 모라토리엄** | 🛑 절단된 스코프를 끝낼 때까지 **새 결정 캐기 중단**(2026-08-19~). 중간 판단은 기본값 채택 후 한 줄 기록. 근거: 결정 이력 15행이 이미 제약 표면이 되어 코드 한 줄마다 충돌 확인이 필요한 상태 |
| **데드라인** | 2026-09-26 |

### 스코프 (2026-08-19 절단 확정)

| 남긴 것 | 뺀 것 (구현 제외) |
|---------|------------------|
| 계좌개설 · 입금 (마무리 중) | 해지 5단계 · 지급 한도 · k6 부하테스트 · 재처리 배치 |
| **이자 계산 — 계산 로직 + 경계값 단위 테스트만** | 이자 배치 · `@Scheduled` · `DAILY_BALANCE_SNAPSHOT` |

> 뺀 것도 **설계는 이 문서와 career-wiki 에 남아 있다** → *"설계했고 스코프에서 뺐다, 이유는 이것"* 으로 설명 가능.
> 이자만 남긴 이유: 구간별 금리·일할·1원 미만 절사가 이 프로젝트에서 밀도가 가장 높은 금융 알고리즘인데
> DB·Flyway 를 안 건드려 **인프라 비용이 0** 이다. **알고리즘만 취하고 그것을 둘러싼 배관은 버린다.**
...
- ~~**지급 한도** — 연령대별 이체 한도 (`PYMT_LMT_POLICY_MST`)~~ → **스코프 제외**(08-19). 설계만 유지
...
> ⚠️ **스코프**: 위 계산식과 경계값 단위 테스트까지만 구현한다(08-19). 결산 스케줄·배치·잔액 스냅샷 테이블은 제외 — 위 "매월 네번째 금요일"은 **설계 기록**이지 구현 대상이 아니다.
...
### 해지 프로세스 (5단계) — ⛔ 스코프 제외 (08-19). 설계 기록으로만 유지
...
└── (없음) ⛔ 이자 테이블 신설 없음 — 08-19 절단으로 이자는 계산 로직만 남음
```

> 미생성 유지: `MONTHLY_LIMT_LEDGER`(월누계는 실시간 SUM — 벤치마크 대기), `TXN_PENDING`/`REFUND_HISTORY`(카톡 스코프), `ACCT_PYMT_RESTR`/`PYMT_LMT_POLICY_MST`(지급 스코프), `DAILY_BALANCE_SNAPSHOT`/`INTEREST_HISTORY`(08-19 이자 절단)
>
> **V6 이 마지막 마이그레이션일 가능성이 높다** — 남은 스코프(락 테스트·`withdraw` 검증·상태전이·이자 계산)는 전부 스키마 무변경이다.

---

## 역할 분담

**판단 기준: "이것을 내가 설명해야 하는가?" → Yes면 사용자가 직접 작성, No면 Claude Code가 작성하고 사용자는 아예 들고 가지 않는다(리뷰도 최소).**

> 2026-08-19 재조정 — 기준의 축을 *"누가 쓰는가"* 에서 **"설명해야 하는가"** 로 옮겼다.
> 08-03 의 실수는 Claude 가 쓴 것이 아니라 **쓴 것을 *설명해야 하는 목록*에 남겨둔 것**이다.
> 리뷰만 한 코드는 오기억이 되고(08-10 실측 2건), **오기억은 빈칸보다 비싸다** — 빈칸은 재독으로
> 복구되지만 오기억은 그 위에 코드를 쌓는다. 따라서 설명 대상이면 **예외 없이** 직접 쓴다.

> 2026-08-04 수정 — 기존 기준은 *"설명을 요구받을 **알고리즘**인가"*였는데, 테스트 전략(슬라이스·Testcontainers·쿼리 검증)처럼 **알고리즘은 아니지만 면접 단골**인 영역이 전부 Claude 쪽으로 오분류됐다. "테스트 어떻게 하셨어요"는 거의 반드시 나온다. 반대로 JWT 필터·WireMock 세팅·DTO 매핑은 실제로 묻지 않으므로 계속 Claude 영역이다.

| 영역 | 담당 | 이유 |
|------|------|------|
| 인증 / Spring Security + JWT / 공통 인프라 | Claude Code 90%+ | 비업무 영역 |
| 배관 (컨트롤러 와이어링, RepositoryImpl, DTO 변환, 단순 상태전이) | Claude Code 작성, 사용자는 **들고 가지 않음** | 설명 대상이 아니므로 리뷰 최소. ⚠️ **TX 경계·롤백 의미론·예외 vs 결과 객체는 여기서 제외** — 면접 정면 소재라 아래 행으로 이동(08-19 소급 재분류) |
| 도메인 알고리즘 (이자 계산, 한도 누적, 동시성) **+ TX 경계·롤백 의미론 + 테스트 전략** | 사용자 작성, Claude는 힌트/검증 | 설명을 요구받는 핵심. 경계값·계산 로직은 직접 증명 |
...
| 계좌 개설 / 입금 | Testcontainers 통합 테스트 | TC JDBC URL 방식 (해지는 스코프 제외) |
| ~~이자 산출 배치~~ | ⛔ 스코프 제외 (08-19) | 이자는 **계산 로직 단위 테스트만** |
| ~~동시 이체 정합성 (k6)~~ | ⛔ 스코프 제외 (08-19) | 동시성은 **비관적 락 인프라 테스트**(스레드 2개)로 증명 |
...
| 2026-08-04 | **학습 방식 점검 + 역할분담 기준 조정** — 현행 유지(근거: 이 레포에 기록된 Claude 오류 교정 5건 / 산출물이 코드가 아니라 "설명할 수 있는 상태"라 이해는 비용이 아님) / 기준을 "설명을 요구받을 **알고리즘**" → **"설명을 요구받을 것"**으로 확장 / 진행 방식 예외: **낯선 것(unfamiliar) vs 처음 보는 장르(novel)** 구분 — 후자는 Claude 선작성 후 읽기·질의, 단 다음 유사 작업은 기본값(직접 작성)으로 복귀 / 일정 압박의 처방은 깊이가 아닌 **스코프 절단** | decisions/2026-08-04-학습방식-역할분담-기준조정.md |
| 2026-08-03 | **입금 오케스트레이션·컨트롤러 경계** — 입금 API는 **대외 수신**(타행이 계좌번호 직접 지정, permitAll)이고 `channelType`은 요청이 지정 / 거절은 4xx + 기존 `ErrorResponse`이며 **컨트롤러가 throw** — 07-30 Q5가 막은 것은 "예외"가 아니라 **"실패 마킹과 같은 TX 안의 예외"**(롤백 경계 밖은 안전) / 오류 vs 거절 판정 기준 = **"고치면 같은 요청이 성공하는가"**(성공→PENDING 유지·재처리 / 아니면→FAILED 마감) / `Account.isActive()` 신설 · `MONTHLY_DEPOSIT_LIMIT_EXCEEDED`→`DEPOSIT_LIMIT_EXCEEDED` 개명 / 전역 예외 핸들러: 예외 삼킴 해소 + 파싱실패 500→400 + `BusinessException`은 5xx만 로깅 | decisions/2026-08-03-입금-오케스트레이션-컨트롤러경계-예외처리.md |
| 2026-08-19 | **스코프 절단·완료 기준 확정** — 완료 기준을 제품에서 **"계좌개설·입금 각각 20분 설명 가능"**으로 재정의 / 해지·지급한도·k6·재처리배치 **구현 제외**, 이자는 **계산 로직 + 경계값 단위 테스트만** / 역할분담 축을 "누가 쓰는가"→**"설명해야 하는가"**로 이동(08-04 기준 확장의 소급 적용) / **grill 모라토리엄** | decisions/2026-08-19-스코프절단-완료기준-확정.md |
