...
| **다음 단계** | ⚠️ **먼저: 완료 기준·스코프 절단 결정**(미결 — 아래 참조). 기술 항목은 **비관적 락 인프라 테스트** 하나 — 스레드 2개가 필요해 기존 `@DataJpaTest` 4종 세트로 불가 → `withdraw` 잔액검증 → 계좌 상태전이 메서드 |
| **미해결(주의)** | **비관적 락 쿼리는 JPQL 파싱까지만 검증** — 실제 락 획득 테스트 없음(월누계 SUM은 8/3 검증 완료) / 🔎 **V6 CHECK에 구멍** — `FAILED` + `fail_reason = ''`(빈 문자열)이 통과한다. `IS NOT NULL`만 보므로 길이 0은 안 걸림. 빈 사유는 추적 근거 부재와 같아 취지에 어긋나지만 `@Enumerated(STRING)`이 `''`를 안 만들어 실위험 낮음 → **V7로 조이지 않고 유지**(8/3 결정, 범위 통제). 참고: 오라클은 `''`=NULL, PostgreSQL은 구분 / 🚧 **완료 기준·스코프 절단 미결(8/10 최우선)** — 목표는 포트폴리오인데 완료 기준이 제품("수신 도메인 구현")이라 07-30·08-04 두 번의 절단 처방이 실행되지 않았다는 진단. 제안: 완료 기준을 "계좌개설·입금 각각 20분 설명 가능"으로 재정의 / 해지·지급한도·k6·재처리배치 절단하되 **이자 계산은 알고리즘 + 경계값 단위테스트만** 남김(배치·스케줄러·스냅샷 제외) / 역할분담을 "누가 쓰는가"→"설명해야 하는가"로 재조정. **결정 전까지 CLAUDE.md 완료기준·스코프·역할분담 항목은 수정 보류** → `career-wiki/wiki/queries/2026-08-10-이해점검-스코프-완료기준-진단.md` / 08-03 변경분 중 Service·컨트롤러는 통합 테스트 2케이스가 덮음(8/10). **전역 예외 핸들러는 여전히 테스트 없음** / `withdraw` 잔액 검증 없음(음수 가능) / 계좌 상태전이 메서드(해지·동결) 부재 → `AccountTest` 비활성 케이스 `@Disabled` 보류 중 / `deposit_limit_policy` D01 시드 금액은 **임시값** / 🕛 **시각 일관성 미판단** — 한 입금 처리에서 "지금"을 3곳이 각각 구함(`DepositProcessService`·`Account.deposit()`·`TransactionHistory` 생성자), 자정 경계에서 당월 누계 기준월과 `txn_dt`가 갈릴 수 있음 / 🧾 `raw_data`는 DTO 재직렬화라 미지 필드가 원본에서 소실 |
...
**판단 기준: "이것이 면접에서 설명을 요구받을 것인가?" → Yes면 사용자, No면 Claude Code.**
